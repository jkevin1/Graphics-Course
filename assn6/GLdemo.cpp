...
extern "C" {
	...
	void keyPress(GLFWwindow *win, int key, int scancode, int action, int mods) {
		...
		if (key == GLFW_KEY_R && action == GLFW_PRESS) {
			...
			appctx->rec.end(appctx->view.cam);
			appctx->recording = false;
			appctx->time = 0.0;
		}
...
int main(int argc, char *argv[]) {
	...
	appctx.rec.start(appctx.view.cam);
	...
	while (!glfwWindowShouldClose(win)) {
		prev = curr;
		curr = glfwGetTime();
		appctx.time += curr - prev;

		if (appctx.recording) {
			// check for updates while a key is pressed
			appctx.redraw |= appctx.input.keyUpdate(appctx.geom, appctx.view, heightmap, *win);
			if (appctx.time >= 1.0) {
				appctx.rec.addFrame(appctx.view.cam);
				appctx.time -= 1.0;
			}
		} else {
			if (appctx.rec.playback((float)appctx.time, appctx.view, win)) {
				printf("Playback complete\n");
				appctx.recording = true;
				appctx.time = 0.0;
				appctx.rec.start(appctx.view.cam);
