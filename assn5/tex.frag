...
// alpha value threshold for reflection
...
//c
const float Rval = pow((0.4 - 1) / (0.4 + 1), 2);
const vec3 R0 = vec3(Rval, Rval, Rval);
...
void main() {
	...
	if (color.a < threshold) {
		...
		vec3 R = R0 + (1 - R0) * pow(1.0 - N_V, 5.);
		//Calculate reflection vector
		vec4 ref = viewInverse * vec4(reflect(V, N), 0.0);
		vec4 reflectedColor = texture(cubemap, ref.xyz);
		fragColor.xyz = mix(fragColor.rgb, reflectedColor.rgb, R);
	}
}
