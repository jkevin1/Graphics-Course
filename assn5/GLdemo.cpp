...
int main(int argc, char *argv[]) {
	int extra_credit, model;
	FILE* readme = fopen("README.txt", "r");
	fscanf(readme, "ExtraCredit=%d\n", &extra_credit);
	fscanf(readme, "Model=%d\n", &model);
	fclose(readme);
	...
	appctx.view.cam.translate(Vec3(0, 2.15f, -1.85f));
	appctx.view.cam.look(0, 0.5f / Y_SENSITIVITY);
	appctx.view.update(win);

	// create stuff to draw
	// comment out to select what to draw
	if (model == 2) {
		OBJModel teapot(appctx.geom, tex, "square.obj", Vec3(0, 0, 0), Vec3(1, 1, 1), Vec3(F_PI / 2, 0, 0));
	} else if (model == 1) {
		Sphere sphere(appctx.geom, tex, 1.0f, Vec3(0, 0, 0));
	} else {
		OBJModel terrain(appctx.geom, tex, "terrain.obj", Vec3(0, 0, 0), Vec3(1, 1, 1), Vec3(F_PI / 2, 0, 0));
	}

	appctx.redraw = true;
	appctx.geom.finalizeDrawData();

	GLint thresholdID = appctx.geom.getUniformLocation("threshold");
	if (!extra_credit) {
		printf("Using threshold of 0.5\n");
		glUniform1f(thresholdID, 0.5f);
	} else {
		printf("EXTRA CREDIT: threshold = (sin(time/2) + 1) / 2\n");
	}

	//Align textures so they match GL_TEXTURE_CUBE_MAP_POSITIVE_X + i
	int order[6] = { 0, 1, 3, 2, 5, 4 };
	Cubemap cubemap("snow%d.pam", order, 9);
	GLint cubemapID = appctx.geom.getUniformLocation("cubemap");
	glUniform1i(cubemapID, 9);

	// loop until GLFW says it's time to quit
	while (!glfwWindowShouldClose(win)) {
		// check for updates while a key is pressed
		appctx.redraw |= appctx.input.keyUpdate(appctx.geom, appctx.view, *win);

		if (extra_credit) {
			float threshold = (sinf((float)glfwGetTime() / 2.0f) + 1.0f) / 2.0f;
