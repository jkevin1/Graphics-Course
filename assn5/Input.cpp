...
bool Input::keyUpdate(Geometry &geom, View &view, GLFWwindow &win) {
	...
	if (glfwGetKey(&win, GLFW_KEY_SPACE) == GLFW_PRESS) moveRate.z -= 25.0f;
	if (glfwGetKey(&win, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS) moveRate.z += 25.0f;

	if (length(moveRate) > 0) {
		Vec<3> pos = view.cam.position;
		view.cam.move(moveRate * dt);
		view.update(&win);
		return true;			// need to redraw
	}
	return false;				// no redraw
