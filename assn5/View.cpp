...
void View::update(GLFWwindow *win) {
	...
	frameUniforms.projMats = Xform::perspective(
		F_PI/4, (float)width/height, 0.01f, 100.0f);

	// update view matrix
	frameUniforms.viewMats = Xform::xrotate(cam.pitch - F_PI / 2) *  Xform::zrotate(cam.yaw) * Xform::translate(cam.position) * Xform::xrotate(-F_PI / 2);
