...
int main(int argc, char *argv[]) {
	...
	Cylinder cyl0(appctx.geom, tex, 50, 100, Vec3(0,0,-50));
	Sphere sph1(appctx.geom, tex, 25, Vec3(-150,0,0));
	Sphere sph2(appctx.geom, tex, 60, Vec3(150, 0, 0));
	OBJModel hex(appctx.geom, tex, "hex.obj", Vec3(0, 150, 0), Vec3(1, 1, 1), Vec3(0, 0, 0));
	OBJModel teapot(appctx.geom, tex, "teapot.obj", Vec3(0, -150, -60), Vec3(100, 100, 100), Vec3(3.14f/2, 0, 0));
	appctx.geom.finalizeDrawData();

	// loop until GLFW says it's time to quit
	while (!glfwWindowShouldClose(win)) {
		// check for updates while a key is pressed
		appctx.redraw |= appctx.input.keyUpdate(appctx.geom, cyl0, *win);
